...
export default function ProductsPage() {
  ...
  const openStockModal = (product: Product) => {
    ...
    setStockFormData({ type: 'ENTRY', quantity: '', reason: '' });
  ...
  const handleStockSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/products/${selectedProduct.id}/stock-logs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          createdBy: 'Admin', // TODO: Kullanıcı sistemi eklendiğinde burası güncellenecek
        }),
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600">Yerel stok ürünlerinizi yönetin</p>
        </div>
        <button
          ...
          <table className="w-full">
            <thead className="border-b border-gray-200 bg-gray-50">
              <tr className="text-left text-sm text-gray-600">
                ...
                <th className="p-4 font-medium">Lokasyon</th>
                <th className="p-4 font-medium">Stok</th>
                <th className="p-4 font-medium">Fiyat</th>
                <th className="p-4 font-medium">Pazaryerleri</th>
              ...
              {products.length === 0 ? (
                ...
                products.map((product) => (
                  <tr key={product.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="p-4 font-mono text-xs text-gray-600">{product.sku}</td>
                        ...
                        {product.mappings.map((mapping) => (
                          <div key={mapping.id} className="relative group">
                            <span 
                              ...
                              title={`Remote SKU: ${mapping.remoteSku}\nRemote ID: ${mapping.remoteProductId || 'N/A'}\nSync: ${mapping.syncStock ? '🔄 Aktif' : '❌ Pasif'}`}
                            >
                              {mapping.marketplace.name}
                              {mapping.syncStock && <span className="ml-1">🔄</span>}
                            </span>
                            
                            {/* Tooltip */}
                            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-10 pointer-events-auto">
                              <div className="text-left">
                                <div><strong>Remote SKU:</strong> {mapping.remoteSku}</div>
                                <div><strong>Remote ID:</strong> {mapping.remoteProductId || 'N/A'}</div>
                                <div><strong>Sync:</strong> {mapping.syncStock ? '🔄 Aktif' : '❌ Pasif'}</div>
                              </div>
                              <div className="mt-2 flex gap-1">
                                {mapping.marketplace.name === 'Trendyol' && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      openTrendyolDetails(product, mapping);
                                    }}
                                    className="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded pointer-events-auto"
                                  >
                                    📋 Detaylar
                                  </button>
                                )}
                                <button
                                  ...
                                  className="px-2 py-1 bg-red-600 hover:bg-red-700 text-white text-xs rounded pointer-events-auto"
                                >
                                  Kaldır
                                </button>
                              </div>
          ...
          <div className="w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-4 rounded-lg bg-blue-50 p-3">
              <p className="text-sm text-blue-900">
                ...
                <span className="font-medium ml-2">Mevcut Stok:</span> {selectedProduct.stockQuantity} adet
              </p>
            </div>
            {stockLogs.length === 0 ? (
